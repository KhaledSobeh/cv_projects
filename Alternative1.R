...
## fruits stand: 30k
### for one minute after 30 minutes from entering to the hall

## shawarma stand: 15k
### every guest, singles, couples and families can eat shawarma.
...
wedding <- simmer("wedding") %>%
  ...
  add_resource("shawarma_stand",capacity= 1, queue_size=Inf) %>%   #7
...
dessert_traj <- trajectory("dessert_traj") %>% 
  ## check fruits stand
  branch(option = function() get_global(wedding,"fruits_stand"),c(F),
         trajectory() %>% set_global("found_dessert",1, mod = "+")) %>%
...
reception_traj <- trajectory("reception_traj") %>%
  ...
  simmer::select(resources=function() nextStand(get_attribute(wedding,keys = c("focaccia_stand","banni_stand", "sushi_stand1", "sushi_stand2", "vegetarian_stand", "tortilla_stand","shawarma_stand"))),
  ...
  rollback(amount = 10, check = function() 7 != sum(get_attribute(wedding,keys = c("focaccia_stand","banni_stand", "sushi_stand1", 
                                                                                   "sushi_stand2", "vegetarian_stand", "tortilla_stand","shawarma_stand")))) %>%
  join(canopy_traj)
...
to_reception_or_canopy_or_hall_traj <- trajectory("to_reception_or_canopy_or_hall_traj") %>%
  ...
  branch(option = function() ifelse(get_attribute(wedding,"guest_type")==0,1,2), continue = c(T,T),
         trajectory() %>% set_attribute(keys = c("focaccia_stand","banni_stand","sushi_stand1", "sushi_stand2",
                                                 "vegetarian_stand", "tortilla_stand","shawarma_stand"), values = c(1,1,0,0,0,0,0)),
         trajectory() %>% set_attribute(keys = c("focaccia_stand","banni_stand","sushi_stand1", "sushi_stand2",
                                                 "vegetarian_stand","tortilla_stand","shawarma_stand"), values = c(0,0,0,0,0,0,0))) %>%
  join(reception_traj)
...
reception_desk_traj <- trajectory("reception_desk_traj") %>%
  ...
  branch(option = function() rdiscrete(1,c(0.07,0.93),c(1,0)),continue=c(F), trajectory()) %>% # 7% came to wrong event
...
canopy_traj_setting <- trajectory("canopy_traj_setting") %>%
  set_global("is_canopy_ended",0) %>% #canopy not started
  ...
  timeout(function() 30) %>%
  set_global("fruits_stand", 1) %>%
  timeout(function() 1) %>%
  set_global("fruits_stand", 0)
...
mm1envs <- mclapply(1:50, function(i) {
...
weddingAttributeData1 <- get_mon_attributes(mm1envs)
weddingArrivalData1 <- get_mon_arrivals(mm1envs, per_resource=T, ongoing = T)


ServedData1 <- sqldf("select max(value) as served
                     from weddingAttributeData1
                     where key = 'served_to'
                     group by replication")

happy1 <- sqldf("select max(value) as dessert
                     from weddingAttributeData1
                     where key = 'found_dessert'
                     group by replication")

(data1 <- data.frame(ServedData1,happy1))
